...
void host_gen_vect()
{
    ...
    if (bit_is_set(UHINT, DDISCI))
    {
        ...
        // Turn off Vbus long enough for the USB core to notice, then turn it back on.
        // This is required for the next device connection to be detected.
        //FIXME - Sometimes it still doesn't work.
        //      Keeping vbus off longer doesn't help.  Clearing SOFEN doesn't help.
...
ISR(TIMER0_OVF_vect)
{
    ...
    {
        PORTB |= 0x10;
    } else if ((usb_state == USB_Connected || usb_state == USB_Broken) && div == 0)
    {
        PORTB |= 0x20;
        PORTB ^= 0x10;
    }
    ...
    {
        int16_t w0, w1, w2, w3;
        if (i == selected_robot && usb_state == USB_Configured)
        {
            // 1  2
            // 0  3
            w0 = -drive_spin - drive_x + drive_y;
            w1 = -drive_spin + drive_x + drive_y;
            w2 = -drive_spin + drive_x - drive_y;
            w3 = -drive_spin - drive_x - drive_y;
...
void host_main()
{
    ...
    // Setup timer 0 for sending forward packets at about 30Hz
    TCCR0B = 0x05;
